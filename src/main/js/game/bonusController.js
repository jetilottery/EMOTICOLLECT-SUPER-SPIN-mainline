...
define([
    ...
    let bonusAnim = ['temp'],
        bonusAnimName = ['temp'];
    let bonusSpinBoxAnim = null,
        isAutoSpin = false,
        bonusEmotiAnimTimer = null,
    ...
    function resetAll() {
        ...
        msgBus.publish("enableBonusSpinButton", true);
        msgBus.publish("enableBonusAutoSpinButton", true);
    ...
    function addMask(isportrait) {
        if (isportrait) { // adjustPosition(gr.lib._revealAll);
    ...
    function startPlay() {
        if (running) return;
        if (isinit) {
            msgBus.publish("enableBonusSpinButton", false);
            msgBus.publish("enableBonusAutoSpinButton", false);
            audio.play('UiSpin', 0);
    ...
    function startBonus() {
        ...
        bonusEmotiAnimTimer = gr.getTimer().setInterval(function() {
            ...
            // EMOTISPIN-203 - TRY - Paytable & Help File are both enabled while 
            // bonus animation is still happening.
            enableConsole();
    ...
    // EMOTISPIN-169 - Game crashes after fast clicking "TOURNER" button in bonus game
    // Channel all enable/disable calls for the SPIN button through here
    function enableBonusSpinButton(enable) {
        spinButton.enable(enable);
        gr.lib._bonusSpinbutton.pixiContainer.interactive = enable;
        gr.lib._bonusSpintext.updateCurrentStyle({ _opacity: enable ? "1" : "0.3" });
        gr.lib._bonusSpinbutton.pixiContainer.scale.x = 1;
        gr.lib._bonusSpinbutton.pixiContainer.scale.y = 1;
    }

    // EMOTISPIN-169 - Game crashes after fast clicking "TOURNER" button in bonus game
    // Channel all enable/disable calls for the AUTO SPIN button through here
    function enableBonusAutoSpinButton(enable) {
        autoSpinButton.enable(enable);
        gr.lib._bonusAutospinbutton.pixiContainer.interactive = enable;
        gr.lib._bonusAutospintext.updateCurrentStyle({ _opacity: enable ? "1" : "0.3" });
        gr.lib._bonusAutospinbutton.pixiContainer.scale.x = 1;
        gr.lib._bonusAutospinbutton.pixiContainer.scale.y = 1;
    }

    msgBus.subscribe("enableBonusSpinButton", enableBonusSpinButton);
    msgBus.subscribe("enableBonusAutoSpinButton", enableBonusAutoSpinButton);
