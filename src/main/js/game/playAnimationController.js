...
define([
    ...
    var prizeTable = {},
        jackpot = 0; //To record ticket prize table
    ...
    var spinButton = null,
        ...
        stopButton = null,
        spinCounter = 0;
    var outWheelRotation = null,
        middleWheelRotation = null,
        innerWheelRotation = null;
    let wheelFlashAnim = null,
        spinBoxAnim = null,
        RotateLight = null,
        collect3Anim = null;
    let spineStyle = null,
    ...
    let outWheelAngleList = null,
        middleWheelAngleList = null,
        innerWheelAngleList = null;
    let freeSpinAnim = null,
        expandAnim = null,
        baseBgAnim = null,
        logoAnim = null,
        bonusAnim = null,
        baseTransitionAnim = null;
    let emotiSequence = [
    ...
    // We need to keep track of the wheels that are spinning
    let arrayOfSpinningWheels = [];
    let initialAnimComplete = false;

    let arrayOfRevealingIW = [];
    ...
    function onGameParametersUpdated() {
        ...
        msgBus.publish("enableSpinButton", false);
        msgBus.publish("enableAutoSpinButton", false);
    ...
    function initialRotary() {
        outWheelRotation = new RotaryTable({
            ...
            callback: () => {
                gr.getTimer().setTimeout(function() {
                    ...
                    checkAllWheelsStopped(outWheelRotation);
                }, 50);
            },
        });
        middleWheelRotation = new RotaryTable({
            ...
            callback: () => {
                ...
                checkAllWheelsStopped(middleWheelRotation);
            },
        });
        innerWheelRotation = new RotaryTable({
            ...
            callback: () => {
                ...
                checkAllWheelsStopped(innerWheelRotation);
            },
        });
    }

    // EMOTISPIN-163 - LNB: The game is displaying X3 win when its going giving the user a X2 win.
    // We *only* want to start processing anything after all the wheels have stopped
    function checkAllWheelsStopped(wheel) {
        arrayOfSpinningWheels.push(wheel);
        if (arrayOfSpinningWheels.length === 3) {
            // All wheels have definitely stopped
            // We can carry on with processing
            switchArrowColor(3);
    ...
    function outWheelBackAnmi() {
        ...
        if (_rotate !== 0) {
            ...
            handleInitialAnimComplete();
        }
    }

    function handleInitialAnimComplete() {
        let autoRevealEnabled = SKBeInstant.config.autoRevealEnabled === false ? false : true;
        initialAnimComplete = true;

        msgBus.publish("enableSpinButton", initialAnimComplete);
        msgBus.publish("enableAutoSpinButton", initialAnimComplete && autoRevealEnabled);
    ...
    function onStartUserInteraction(data) {
        ...
        if (autoRevealEnabled) {
            msgBus.publish("enableAutoSpinButton", initialAnimComplete);
    ...
    function initialBaseGame() {
        ...
        // Add listeners, absolutely no harm in doing it like this
        instantWinButton[0].click(function() {
            disableButtonsAndReveal(0);
        });
        instantWinButton[1].click(function() {
            disableButtonsAndReveal(1);
        });
        instantWinButton[2].click(function() {
            disableButtonsAndReveal(2);
        });
    }

    // EMOTISPIN-167 - LNB - Error 29000 triggered during gameplay
    // Make sure we 100% disable the buttons before reacting to the one we just clicked
    function disableButtonsAndReveal(symbol) {
        if (arrayOfRevealingIW.length !== 0) {
            return;
        }
        arrayOfRevealingIW.push(symbol);
        for (let j = 0; j < 3; j++) {
            gr.lib["_symbolInstant_Top" + j].pixiContainer.interactive = false;
            gr.lib["_symbolInstant_Top" + j].pixiContainer.cursor = "default";
        }
        // Now handle reveal
        handleInstantWinButtonReveal(symbol);
    }

    // EMOTISPIN-167 - LNB - Error 29000 triggered during gameplay
    // Separated function into two parts
    function handleInstantWinButtonReveal(symbol) {
        gr.lib._iwText.show(false);
        gr.lib._iwArrow.show(false);
        gr.lib._iwTextbg.show(false);
        ...
        for (let j = 0; j < 3; j++) {
            if (symbol !== j) {
                gr.lib["_Top" + j + "_BG"].gotoAndPlay("instant_Win_outro1", 0.2, false);
                gr.lib["_Top" + j + "_BG"].onComplete = function() {
                    gr.lib["_Top" + j + "_BG"].show(false);
                };
            } else {
                gr.lib["_Top" + j + "_BG"].gotoAndPlay("instant_Win_spin", 0.2, false);
                gr.lib["_Top" + j + "_BG"].onComplete = function() {
                    gr.lib["_Top" + j + "_value"].show(true);
                    gr.lib["_Top" + j + "_value"].setText(SKBeInstant.formatCurrency(iwMoney).formattedAmount);
                    gr.getTimer().setTimeout(function() {
                        handleRevealAnimComplete(j);
                    }, 1000);
                };
            }
        }
    ...
    // EMOTISPIN-167 - LNB - Error 29000 triggered during gameplay
    // Separated function into two parts
    // Now setting onComplete to null before setting another anim
    function handleRevealAnimComplete(j) {
        gr.lib["_Top" + j + "_BG"].onComplete = null;
        gr.lib["_Top" + j + "_BG"].gotoAndPlay("instant_Win_outro2", 0.2, false);
        gr.lib["_Top" + j + "_value"].show(false);
        let temp = j;
        if (isSecond) {
            temp = j == 0 ? "3" : "4";
        }
        let tempName = gr.getSize().height > gr.getSize().width ? "_P_valueTop" : "_valueTop";
        if (gr.getSize().height > gr.getSize().width) {
            gr.lib["_Top" + temp + "_value1_P"].show(true);
            gr.lib["_Top" + temp + "_value1_P"].setText(SKBeInstant.formatCurrency(iwMoney).formattedAmount);
        } else {
            gr.lib["_Top" + temp + "_value1"].show(true);
            gr.lib["_Top" + temp + "_value1"].setText(SKBeInstant.formatCurrency(iwMoney).formattedAmount);
        }
        ...
        gr.animMap[tempName + temp + "_Anim"]._onComplete = function() {
            ...
            if (jackpot > winValue) {
                ...
                gr.getTimer().setTimeout(function() {
                    // Proceed after win value updated
                    proceedAfterWinValueUpdated();
                }, 500);
            }
        };
    }

    // EMOTISPIN-164: LNB: When Hardwar acceleration is off the Picker bonus is not adding to the game
    function proceedAfterWinValueUpdated() {
        recoveryInstantWin();
    ...
    function spin() {
        // EMOTISPIN-170 - Game displays incorrect "TOUR RESTANT" (turns left) when player rapidly clicks TOURNER immediately after clicking AUTOPLAY
        // Check to see if a turn is in progress first
        if (turnInProgress) {
            // Turn already in progress, return
            return;
        }
    ...
    function outAndMiddleCollectionAnim() {
        ...
        for (let i = 1; i <= 3; i++) {
            ...
            Tween.to(
                gr.lib["_emoti_collect_" + i], { scale: { x: 0.7, y: 0.7 } },
                300, {
            ...
            gameUtils.playSpine(
                ...
                function() {
                    ...
                    for (let i = 1; i <= 3; i++) {
                        ...
                        Tween.to(
                            gr.lib["_emoti_collect_" + i], { scale: { x: 0.6, y: 0.6 } },
                            300, {
                                _onComplete: function() {
        ...
        if (features.length === 0) {
            ...
            if (isAutoSpin) {
                ...
                if (isLastSpin() === false) {
                    ...
                    msgBus.publish("enableSpinButton", true);
                    msgBus.publish("enableAutoSpinButton", true);
                }
    ...
    function adjustSpinBotton(portrait) {
        if (portrait) {
            gr.lib._spinButton.updateCurrentStyle({ _left: 130 });
            //gr.lib.spinButton, { x: 130 }, 0);

        } else {
            if (onePrice) {
                gr.lib._spinButton.updateCurrentStyle({ _left: 150 });
                //Tween.to(gr.lib.spinButton, { x: 150 }, 0);

            } else {
                gr.lib._spinButton.updateCurrentStyle({ _left: 0 });
                //Tween.to(gr.lib.spinButton, { x: 0 }, 0);
    ...
    // EMOTISPIN-168 - Game becomes unresponsive after fast clicking "TOURNER" button in base game
    // Channel all enable/disable calls for the SPIN button through here
    function enableSpinButton(enable) {
        spinButton.enable(enable);
        gr.lib._spinButton.pixiContainer.interactive = enable;
        gr.lib._spinText.updateCurrentStyle({ _opacity: enable ? "1" : "0.3" });
        gr.lib._spinButton.pixiContainer.scale.x = 1;
        gr.lib._spinButton.pixiContainer.scale.y = 1;
    }

    // EMOTISPIN-168 - Game becomes unresponsive after fast clicking "TOURNER" button in base game
    // Channel all enable/disable calls for the AUTO SPIN button through here
    function enableAutoSpinButton(enable) {
        autoSpinButton.enable(enable);
        gr.lib._autospinButton.pixiContainer.interactive = enable;
        gr.lib._autospinText.updateCurrentStyle({ _opacity: enable ? "1" : "0.3" });
        gr.lib._autospinButton.pixiContainer.scale.x = 1;
        gr.lib._autospinButton.pixiContainer.scale.y = 1;
    }

    msgBus.subscribe("enableSpinButton", enableSpinButton);
    msgBus.subscribe("enableAutoSpinButton", enableAutoSpinButton);
    ...
    document.addEventListener('visibilitychange', function() {
        isSwitchTab = gr.lib._winBoxError ? gr.lib._winBoxError.pixiContainer.visible : false;
    });
    return {};
});